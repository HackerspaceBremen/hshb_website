  ...
  class myAtomParser
  {
    ...
    function startElement($parser, $tagname, $attrs=array())
    {
      if($this->encoding) {
        ...
        $tmpdata = "<$tagname";
        if(isset($attrs)) foreach($attrs as $key => $val) $tmpdata .= " $key=\"$val\"";
        $tmpdata .= ">";
        $this->parseData($parser, $tmpdata);
      } else {
        if(isset($attrs['HREF']) && $attrs['HREF'] && $attrs['REL'] && $attrs['REL'] == 'alternate') {
          $this->startElement($parser, 'LINK', array());
          $this->parseData($parser, $attrs['HREF']);
          $this->endElement($parser, 'LINK');
        }
        if(isset($attrs['TYPE'])) $this->encoding[$tagname] = $attrs['TYPE'];
        ...
        if(preg_match("/^(FEED|ENTRY)$/", $tagname)) {
          if($this->tags) {
            ...
            if(isset($parent)) $this->tags[$depth-1][$parent][$tagname]++;
          }
    ...
    function parseData($parser, $data)
    {
      ...
      foreach($this->tags as $tag) {
        if(is_array($tag)) {
          ...
          if(isset($indexes)) extract($indexes);
        } else {
...
//      }
    ...
    function display_entry($data, $parent)
    {
      ...
      // DWF: THIS HTML-STRING IS INSERTING TR/TD-CODES AND NEWS-ICON; OLD:  $this->retval .=  "<p><b>";
      $this->retval .=  "<tr><td width=52><img src=icon.png></img></td><td>";
      if(isset($LINK)) $this->retval .=  "<a href=\"$LINK\" target=\"_blank\"><h2>";
      $this->retval .= stripslashes($TITLE);
      if(isset($LINK)) $this->retval .= "</h2></a>";
      $this->retval .=  "</b>";
      if(isset($ISSUED)) $this->retval .= " <small>($ISSUED)</small>";
      $this->retval .=  "</p>\n";
    ...
    // display entire feed as HTML - DWF: limit auf 15 gesetzt, war vorher false
    function getOutput($limit=15, $output_encoding='UTF-8')
    {
